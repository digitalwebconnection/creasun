...
const SEGMENTS_DATA: SegmentData[] = [
  {
    ...
    points: [
      "1–15 kW typical size",
      "Fast ROI with PM Surya Ghar",
      "Neat wiring & aesthetics",
    ...
    details: {
      size: "1–15 kW",
      capex: "55,000",
      benefit: "Subsidy & savings on electricity bills",
      imagePrompt:
        "photorealistic image of a modern house with sleek solar panels seamlessly integrated into its rooftop, bright sunny day",
    },
  },
  {
    key: "com",
    ...
    points: [
      "20–200 kW typical size",
      "Accelerated depreciation benefits",
      "Tariff & demand charge optimization",
      "Scalable solutions",
    ],
    price: "₹48,000/kW*",
    details: {
      size: "20–200 kW",
      capex: "48,000",
      benefit: "Significant operating cost reduction",
      imagePrompt:
        "photorealistic image of a large, flat commercial building roof with extensive solar panel installation, urban skyline in background",
    },
  },
  {
    key: "ind",
    ...
    activeClass:
      "bg-indigo-100/70 border-indigo-500 text-indigo-900 shadow-lg",
    points: [
      "200 kW – multi-MWp",
      "Structure and operational safety first",
      "SLA-backed O&M for uptime",
    ...
    details: {
      size: "200 kW – MWp",
      capex: "45,000",
      benefit: "Lowest Levelized Cost of Energy (LCOE)",
      imagePrompt:
        "photorealistic image of a vast industrial factory shed roof covered with solar panels, power lines in distance",
    },
  },
  {
    key: "gm",
    ...
    activeClass:
      "bg-emerald-100/70 border-emerald-500 text-emerald-900 shadow-lg",
    points: [
      "Space-rich sites and open land",
      "Best LCOE achievable at scale",
      "Civil and electrical trenching expertise",
    ...
    details: {
      size: "MWp",
      capex: "43,000",
      benefit: "Maximum scale and highest energy output",
      imagePrompt:
        "photorealistic image of a large ground-mounted solar farm with rows of panels stretching into the horizon, clear sky",
    },
  },
];
...
export default function SegmentsAndMountingShowcase() {
  ...
  // 🔄 Auto-scroll effect (switch segment every 4 seconds)
  useEffect(() => {
    const interval = setInterval(() => {
      setActiveSegment((prev) => {
        const index = SEGMENTS_DATA.findIndex((s) => s.key === prev);
        const nextIndex = (index + 1) % SEGMENTS_DATA.length;
        return SEGMENTS_DATA[nextIndex].key;
      });
    }, 3000);
              ...
              <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-600 flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-base text-emerald-900 font-medium">
                    {activeData.details.benefit}
                  </div>
                </div>
              </div>
            </div>

            {/* Column 2: Features & CTA */}
            <div className="md:border-l md:pl-8">
              <h3
                className={`text-2xl font-extrabold ${
                  SEGMENTS_DATA.find((d) => d.key === activeSegment)!.colorClass.split(
                    " "
                  )[0]
                }`}
              >
              ...
              <div className="mt-6 pt-4 border-t border-slate-200">
                ...
                >
                  Start Project for {activeData.price}
                  <DollarSign className="h-5 w-5" />
                </a>
                <p className="mt-3 text-xs text-slate-500">
                  *Indicative CAPEX (per kW), excludes GST/taxes. Final scope
                  and structure depend on site survey.
                </p>
              </div>
            </div>
          </div>
...
function TabButton({
  ...
  const activeStyles = isActive
    ? segment.activeClass
    : `border-transparent text-slate-600 ${segment.colorClass} hover:border-slate-300`;
  ...
  return (
    ...
    >
      ...
      <span className="mt-1 text-sm font-semibold whitespace-nowrap">
        {segment.title.replace("Rooftop", "Roof")}
      </span>
    </button>
  );
}
